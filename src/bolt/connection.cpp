...
void BoltConnection::DoSend() {
    ...
    auto cb = [this](const boost::system::error_code& ec, std::size_t) {
        if (ec) {
            LOG_WARN("async write error: {}, clear {} pending message",
...
void BoltConnection::ReadVersionNegotiationDone(const boost::system::error_code &ec) {
    ...
    if (spdlog::get_level() <= spdlog::level::debug) {
        for (int i = 0; i < 4; i++) {
            LOG_DEBUG("protocol version {} major:{}, minor:{}",
                std::to_string(i), (int)buffer16_[i*4+3], (int)buffer16_[i*4+2]);
        }
    }
    if (!match) {
