...
#define CHECK_DB_NOT_NULL()                                  \
    do {                                                     \
        if (!db_) THROW_CODE(InvalidGalaxyError);            \
    } while (0)

#define CHECK_DB_AND_USER()                                                            \
    do {                                                                               \
        if (!db_) THROW_CODE(InvalidGalaxyError);                                      \
        if (user_.empty()) THROW_CODE(UnauthorizedError, "User is not authorized yet."); \
    } while (0)
...
void lgraph_api::Galaxy::SetCurrentUser(const std::string& user, const std::string& password) {
    ...
    if (token.empty()) THROW_CODE(UnauthorizedError, "Bad user/password.");
    user_ = user;
}
