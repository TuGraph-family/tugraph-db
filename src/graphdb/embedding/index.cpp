...
void Index::Add(const DataSet& ds) {
  ...
  if (vid >= ingest_chunk_->GetMinVId()) {
    ...
    THROW_CODE(InvalidParameter, "Cannot find proper chunk for vid {}", vid);
  }
...
void Index::Delete(const DataSet& ds) {
  ...
  if (ds.n != 1) {
    THROW_CODE(InvalidParameter, "Delete one item each time, ds.n = {}", ds.n);
  }
...
void Index::Search(const DataSet& ds, const SearchParams& params) {
  ...
  for (size_t i = 0; i < ds.k; i++) {
    if (ds.vids[i] != -1) {
      res.emplace_back(ds.vids[i], reinterpret_cast<float*>(ds.distances)[i]);
    }
  }
...
void Index::Compaction() {
  ...
  auto s = db_->Put(wo, graph_cf_->vector_index_manifest, meta_.path(),
...
IndexChunk* Index::GetTargetPersistIndexChunk(int64_t vid) {
  for (auto it = persist_chunks_.rbegin(); it != persist_chunks_.rend(); it++) {
    auto& p_chunk = *it;
