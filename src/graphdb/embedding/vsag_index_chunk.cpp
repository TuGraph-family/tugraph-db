...
class VsagIdSelector : public IdSelector {
 public:
  ...
  bool is_member(int64_t id) const override { return !del_map_->IsDeleted(id); }
...
class VsagHNSWIndexChunk : public IndexChunk {
 public:
  VsagHNSWIndexChunk(rocksdb::TransactionDB* db, GraphCF* graph_cf,
                     const meta::VertexVectorIndex& meta, std::string chunk_id,
    ...
    if (ret.has_value()) {
      ...
      THROW_CODE(VectorIndexException, "Failed to create vsag hnsw index");
    }
  }
...
void VsagHNSWIndexChunk::Add(const DataSet& ds) {
  ...
  if (delta_ == nullptr) {
    delta_ = std::make_unique<Delta>(meta_.dimensions(), meta_.distance_type());
  }
...
void VsagHNSWIndexChunk::WriteToFile() {
  std::filesystem::path target_path =
      fmt::format("{}/{}.{}", meta_.path(), chunk_id_, vsag_hnsw_index_postfix);
...
void VsagHNSWIndexChunk::Seal() {
  WriteToFile();
  mapper_->WriteToFile(meta_.path());
}
...
int64_t VsagHNSWIndexChunk::ApplyDelta() {
  ...
  if (!updates.empty()) {
    ...
    for (auto vid : updates) {
      ...
      property_key.clear();
      property_val.clear();
      property_key.append((const char*)&bigendian_vid, sizeof(bigendian_vid));
      property_key.append((const char*)&bigendian_pid, sizeof(bigendian_pid));
      auto s =
      ...
      for (size_t i = 0; i < array.size(); i++) {
        if (array[i].IsDouble()) {
          embeddings.push_back(static_cast<float>(array[i].AsDouble()));
        } else {
...
std::unique_ptr<VsagHNSWIndexChunk> VsagHNSWIndexChunk::Load(
  ...
  auto file_path =
      fmt::format("{}/{}.{}", meta.path(), chunk_id, vsag_hnsw_index_postfix);
  ...
  std::unique_ptr<IdMapper> mapper = IdMapper::Load(meta.path(), chunk_id);
  if (mapper == nullptr) {
...
std::unique_ptr<IndexChunk> CreateVsagIndexChunk(
  ...
  switch (meta.index_type()) {
    case meta::VectorIndexType::HNSW: {
      ...
      auto metric_type = meta::VectorDistanceType_Name(meta.distance_type());
      ...
      nlohmann::json index_params{{"dtype", "float32"},
                                  ...
                                  {"dim", meta.dimensions()},
                                  {"hnsw", hnsw_params}};
      return std::make_unique<VsagHNSWIndexChunk>(db, graph_cf, meta, chunk_id,
                                                  index_params.dump());
    }
    default:
