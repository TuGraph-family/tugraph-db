...
void checkedAnyCast(const std::any &s, TargetType *&d, const std::string& file, int line) {
    try {
...
std::any CypherBaseVisitorV2::visitOC_RelationshipTypes(
    ...
    if (VisitGuard::InClause(VisitType::kUpdatingPattern, visit_types_)) {
        if (label_strs.size() != 1) {
            THROW_CODE(CypherException, "Create a relationship with more than one type");
        }
    }
    if (label_strs.size() == 1) {
        auto l = ALLOC_GEAOBJECT(geax::frontend::SingleLabel);
...
std::any CypherBaseVisitorV2::visitOC_StringListNullOperatorExpression(
    ...
    if (!ctx->oC_NullOperatorExpression().empty()) {
        ...
        if (list_op_expr->IN()) {
            ...
            AnyCastOrThrow(visit(list_op_expr->oC_PropertyOrLabelsExpression()), right);
...
std::any CypherBaseVisitorV2::visitOC_ReservedWord(LcypherParser::OC_ReservedWordContext *) {
    NOT_SUPPORT_AND_THROW();
    return 0;
}
...
std::any CypherBaseVisitorV2::visitOC_LeftArrowHead(LcypherParser::OC_LeftArrowHeadContext *) {
    NOT_SUPPORT_AND_THROW();
    return 0;
}

std::any CypherBaseVisitorV2::visitOC_RightArrowHead(LcypherParser::OC_RightArrowHeadContext *) {
    NOT_SUPPORT_AND_THROW();
    return 0;
}
