...
class Filter {
 public:
  enum Type {
    ...
    // Filter, and will be replaced with AstExpr in the future.
    GEAX_EXPR_FILTER,
  };
  ...
  /* For filter tree: If there are sub-filter(s) completely contained in
   * ALIASES. For leaf filter: If the filter is completely contained in ALIASES.
   * */
  ...
  bool IsLeaf() const {
    return _type != EMPTY && _type != UNARY && _type != BINARY;
  }
   ...
   * */
  static void RemoveAlias(std::shared_ptr<Filter> &f,
                          const std::vector<std::string> &alias) {
    RemoveFilterWhen(f, [&alias](const auto &b, const auto &e) {
      std::vector<std::string> vs;
      std::set_intersection(b, e, alias.cbegin(), alias.cend(),
...
class GeaxExprFilter : public Filter {
 ...
 public:
  GeaxExprFilter(geax::frontend::Expr *expr,
                 const cypher::SymbolTable &sym_tab) {
