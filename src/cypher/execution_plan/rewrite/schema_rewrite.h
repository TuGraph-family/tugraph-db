...
public:
    ...
    std::vector<cypher::SchemaGraphMap> GetEffectivePath(const lgraph::SchemaInfo &schema_info,cypher::SchemaNodeMap* schema_node_map,cypher::SchemaRelpMap* schema_relp_map);
    ...
    std::map<size_t,std::set<size_t>> GetNextTVidByEdgeId(std::set<size_t>& edge_ids,size_t t_vid);
