...
void static ExtractLabelTree(std::vector<std::string> &labels,
...
void OpGqlMerge::MergeNode(RTContext *ctx,
                           const geax::frontend::Node *node_pattern) {
  std::unordered_map<std::string, Value> properties_merge, properties_on_match,
      properties_on_create;
  ...
  bool on_match = false;
  for (auto iter = ctx->txn_->NewVertexIterator(merge_label, merge_props);
  ...
  if (!on_match) {
    ...
    v = ctx->txn_->CreateVertex({labels.begin(), labels.end()},
                                properties_merge);
  ...
  for (auto chain : path_pattern_->chains()) {
    ...
    if (pattern_element_chains.empty()) {  // create vertex
      ...
      for (auto &element_chainchain : pattern_element_chains) {
        ...
        if (!pattern_graph_->GetNode(start_node->filler()->v().value())
                 .Visited()) {
        ...
        if (!pattern_graph_->GetNode(end_node->filler()->v().value())
                 .Visited()) {
...
void OpGqlMerge::ExtractProperties(
    RTContext *ctx, std::unordered_map<std::string, Value> &ext_props,
...
void OpGqlMerge::ExtractProperties(
    RTContext *ctx, const std::string &var,
    std::unordered_map<std::string, Value> &ex_props,
...
void OpGqlMerge::MergeChains(RTContext *ctx,
                             const geax::frontend::Node *node_patt,
  ...
  if (direction == geax::frontend::EdgeDirection::kPointLeft) {
    ...
    THROW_CODE(CypherException,
               "Only directed relationships are supported in merge");
  }
  ...
  if (edge_labels.empty())
    THROW_CODE(CypherException, "edge label missing in merge");
  auto &src_node = pattern_graph_->GetNode(src_node_var);
  auto &dst_node = pattern_graph_->GetNode(
      dst_node_var);  // get node.Lable() get node.Prop()
  ...
  bool matched = false;
  for (auto iter = src_node.vertex_->NewEdgeIterator(
           graphdb::EdgeDirection::OUTGOING, label, edge_properties,
           dst_node.vertex_.value());
       iter->Valid(); iter->Next()) {
    matched = true;
    /* ON MATCH */
  ...
  if (!matched) {
    ...
    /* ON CREATE */
    e = ctx->txn_->CreateEdge(src_node.vertex_.value(),
                              dst_node.vertex_.value(), label, edge_properties);
