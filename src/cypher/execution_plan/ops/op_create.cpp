...
void OpGqlCreate::ExtractProperties(
    RTContext* ctx, std::unordered_map<std::string, Value>& properties,
...
void OpGqlCreate::CreateVertex(RTContext* ctx, geax::frontend::Node* node,
  ...
  auto v = ctx->txn_->CreateVertex(
      std::unordered_set<std::string>{labels.begin(), labels.end()},
      properties);
...
void OpGqlCreate::CreateEdge(RTContext* ctx, geax::frontend::Node* start,
                             geax::frontend::Edge* edge,
                             geax::frontend::Node* end) {
  ...
  if (!src_node.vertex_ || !dst_node.vertex_) {
    THROW_CODE(CypherException, "Invalid vertex when create edge [{}]: {}",
               edge_variable, !src_node.vertex_ ? src_variable : dst_variable);
  }
  ...
  auto e = ctx->txn_->CreateEdge(src_node.vertex_.value(),
                                 dst_node.vertex_.value(), labels[0], props);
...
void OpGqlCreate::ResultSummary(RTContext* ctx) {
  if (summary_) {
    ...
    header.clear();
    header.emplace_back(std::make_pair("<SUMMARY>",
    lgraph_api::LGraphType::STRING)); ctx->result_->ResetHeader(header);*/
