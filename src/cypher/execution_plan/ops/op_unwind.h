...
class Unwind : public OpBase {
  std::vector<Value> list_;  // List which the unwind operation is performed on.
  ...
  static const unsigned int INDEX_NOT_SET =
 ...
 public:
  Unwind(const ArithExprNode &exp, const std::string &resolved_name,
  ...
  OpResult Initialize(RTContext *ctx) override {
    ...
    if (children.empty()) {
      ...
      if (list.IsArray()) {
        list_ = list.constant.AsArray();
        list_idx_ = 0;
      } else {
        /* Attempting to use UNWIND on an expression that does not return a list
         * such as UNWIND will cause an error. The exception to this is when the
        ...
        if (!list.IsNull())
          THROW_CODE(CypherException, "List expected in UNWIND");
  ...
  std::string ToString() const override {
    std::string str(name);
    str.append(" [").append(resolved_name_).append("]");
    // str.append("
    // [").append(exp_.ToString()).append(",").append(resolved_name_).append("]");
    return str;
  }
