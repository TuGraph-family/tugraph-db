...
void ExpandAll::ExtractProperties(
...
ExpandAll::ExpandAll(PatternGraph *pattern_graph, Node *start, Node *neighbor,
                     Relationship *relp)
...
void ExpandAll::_InitializeEdgeIter(RTContext *ctx) {
  ...
  std::unordered_set<std::string> neighbor_labels = {labels.begin(),
                                                     labels.end()};
    ...
    case ExpandTowards::FORWARD:
      if (expand_into_) {
        eit_ = start_->vertex_->NewEdgeIterator(
            ...
            relp_map, neighbor_->vertex_.value());
      } else {
        eit_ = start_->vertex_->NewEdgeIterator(
            EdgeDirection::OUTGOING,
            std::unordered_set<std::string>{types.begin(), types.end()},
...
std::string ExpandAll::ToString() const {
  auto towards = expand_direction_ == FORWARD    ? "-->"
                 : expand_direction_ == REVERSED ? "<--"
                                                 : "--";
  return fmt::format("{}({}) [{} {} {}]", name, expand_into_ ? "Into" : "All",
                     start_->Alias(), towards, neighbor_->Alias());
}
