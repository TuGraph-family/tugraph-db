...
class OpGqlMerge : public OpBase {
  ...
  void ResultSummary(RTContext *ctx) {
    if (summary_) {
      ...
      summary.append("merged ")
          .append(
              std::to_string(ctx->result_info_->statistics.vertices_created))
 ...
 public:
  OpGqlMerge(const std::vector<geax::frontend::SetStatement *> &on_match_items,
             ...
             const geax::frontend::PathPattern *path_pattern,
    ...
    for (auto &node : pattern_graph_->GetNodes()) {
      ...
      for (auto rr : node.RhsRelps()) {
        ...
        if (r.derivation_ == Relationship::MERGED)
          modifies.emplace_back(r.Alias());
      }
    }
  }
  ...
  void ExtractProperties(RTContext *ctx,
                         std::unordered_map<std::string, Value> &ext_props,
