...
enum OpType {
  ...
  UNION,
  NODE_BY_ID_SEEK,
  // TODO(lingsu): the operator and ast will be decoupled in the future, and ast
  // will generate symbolic information and expression, then the operator will
  // complete the calculation through the symbolic information and expression.
  // and then the operator will be unified, without distinguishing gql or cypher
  GQL_STANDALONE_CALL,
  GQL_CREATE,
  GQL_DELETE,
  GQL_UPDATE,
...
struct OpBase {
  ...
  void InsertChild(const std::vector<OpBase *>::const_iterator &position,
                   OpBase *child) {
  ...
  void PushBelow(OpBase *op) {
    ...
    if (!op->children.empty() || op->parent || !parent) {
      THROW_CODE(CypherException, "internal error");
    }
  ...
  bool IsTap() const {
    return type == OpType::NODE_SCAN || type == OpType::NODE_BY_LABEL_SCAN ||
  ...
  bool IsDynamicScan() const {
    return type == OpType::NODE_INDEX_SEEK_DYNAMIC ||
           type == OpType::ALL_NODE_SCAN_DYNAMIC ||
  ...
  bool IsStreamRoot() const {
    return type == OpType::PROJECT ||
           ...
           || type == OpType::CREATE || type == OpType::DELETE_ ||
           type == OpType::UPDATE || type == OpType::GQL_CREATE ||
           type == OpType::GQL_DELETE || type == OpType::GQL_UPDATE;
  }
  ...
  static void DumpStream(OpBase *op, int indent, bool statistics,
                         std::string &s) {
    if (!op) return;
    ...
    if (statistics) {
      s.append(" (")
          .append(std::to_string(op->stats.profileRecordCount))
          .append(" rows)");
    }
    s.append("\n");
    for (auto child : op->children) {
...
inline std::string ToString(OpBase::OpResult opt) {
  switch (opt) {
    case OpBase::OpResult::OP_DEPLETED:
      return "OP_DEPLETED";
    case OpBase::OpResult::OP_REFRESH:
      return "OP_REFRESH";
    case OpBase::OpResult::OP_OK:
      return "OP_OK";
    case OpBase::OpResult::OP_ERR:
      return "OP_ERR";
    default:
      return "unkown OpResult type";
  }
}
