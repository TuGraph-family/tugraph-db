...
class Traversal : public OpBase {
    ...
    /** about expand */
    std::vector<std::tuple<cypher::Node*, cypher::Node*, cypher::Relationship*, ExpandTowards, bool>> expands_; // <start, neighbor, relp, towards, has_filter>
    ...
    bool DoFilter(lgraph_api::VertexIterator &vit, std::shared_ptr<lgraph::Filter> f) {
        ...
        case lgraph::LBR_EMPTY: {
            ...
            if (prop_name.empty()) { CYPHER_TODO(); }
        ...
        auto vs = lgraph_api::traversal::FindVertices(
            ...
            [&](lgraph_api::VertexIterator &vit) { 
                if (vit.GetLabel() == start_label_ && DoFilter(vit, filter_)) {
                    if (expands_.empty()) {
                        auto group_by_key = noneagg_property_.empty() ? lgraph::FieldData() : vit[noneagg_property_];
            ...
            true);
        // 2. ExpandEdges
        auto ft = lgraph_api::traversal::FrontierTraversal(db_, txn, 
                lgraph_api::traversal::TRAVERSAL_PARALLEL | lgraph_api::traversal::TRAVERSAL_ALLOW_REVISITS);
        ft.SetFrontier(vs); 
            ...
            case ExpandTowards::FORWARD: // out
                ft.ExpandOutEdges(
                    [&](lgraph_api::OutEdgeIterator &eit) { 
                        return std::get<2>(expands_[i])->Types().find(eit.GetLabel()) != std::get<2>(expands_[i])->Types().end();
                    },
                    [&](lgraph_api::VertexIterator &vit) { 
    ...
    OpResult HandOff() {
        ...
        for (auto &col : result_set_header_.colums) {
            if (col.aggregated) {
                record->values[key_idx + agg_idx] = Entry(cypher::FieldData(result_buffer_.at(result_idx_)));
                agg_idx++;
            } else {
                record->values[key_idx + agg_idx] = Entry(cypher::FieldData(result_buffer_.at(result_idx_+1)));
                key_idx++;
            }
        }
...
public:
    Traversal(std::string label, std::string alias,
              ResultSetHeader result_set_header, ArithExprNode agg_expr, ArithExprNode noneagg_expr,
              std::vector<std::tuple<cypher::Node*, cypher::Node*, cypher::Relationship*, ExpandTowards, bool>> expands 
                = std::vector<std::tuple<cypher::Node*, cypher::Node*, cypher::Relationship*, ExpandTowards, bool>>{}
              )
    ...
    void SetFilter(std::shared_ptr<lgraph::Filter> filter) {
        filter_ = filter;
    }
