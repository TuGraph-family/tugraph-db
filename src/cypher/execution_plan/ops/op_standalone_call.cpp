...
OpBase::OpResult GqlStandaloneCall::Initialize(RTContext *ctx) {
  record =
      std::make_shared<Record>(yield_.value()->items().size(), &symbol_table_);
  return OP_OK;
}

OpBase::OpResult GqlStandaloneCall::HandOff(RTContext *ctx,
...
cypher::OpBase::OpResult cypher::GqlStandaloneCall::RealConsume(
    RTContext *ctx) {
