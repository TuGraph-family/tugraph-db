  ...
  for (auto &col : result_set_header_.colums) {
    if (col.aggregated) {
      ...
      if (agg_exp.type == ArithExprNode::AR_EXP_OP &&
        ...
        int opd =
            agg_exp.op.children[0].op.type == ArithOpNode::AR_OP_AGGREGATE ? 1
            : agg_exp.op.children[1].op.type == ArithOpNode::AR_OP_AGGREGATE
                ? 0
                : -1;
        if (opd < 0) CYPHER_TODO();
        if (agg_exp.op.children[opd].operand.type ==
            ArithOperandNode::AR_OPERAND_VARIADIC) {
          auto it = sym_tab_.symbols.find(
              agg_exp.op.children[opd].operand.variadic.alias);
      ...
      record->values[key_idx + agg_idx] =
          agg_exp.Evaluate(ctx, *children[0]->record);
      agg_idx++;
    } else {
...
Aggregate::Aggregate(
    const std::vector<ArithExprNode> &aggregated_expressions,
    ...
    : OpBase(OpType::AGGREGATE, "Aggregate"),
      sym_tab_(*sym_tab),
      result_set_header_(header) {
  ...
  if (state_ == Initialized) {
    ...
    if (group_cache_.empty() && noneaggregated_expressions_.empty()) {
      ...
      for (auto _ : noneaggregated_expressions_) {
        CYPHER_TODO();
        new_group.keys.emplace_back(Value());
      }
      for (auto &ae : aggregated_expressions_) {
        ArithExprNode new_ae(ae.expr_, sym_tab_);
        new_group.aggregation_functions.emplace_back(new_ae);
...
const std::vector<std::string> &Aggregate::AggrItemNames() const {
  return aggr_item_names_;
}

const std::vector<std::string> &Aggregate::ItemNames() const {
  return item_names_;
}
