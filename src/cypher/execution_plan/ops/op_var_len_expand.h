...
#define VAR_LEN_EXP_DUMP_FOR_DEBUG()                                       \
  do {                                                                     \
    SPDLOG_DEBUG(__func__ + __LINE__ + ": hop=" + hop_ + ", edge=" +       \
                 (hop_ == 0 ? "na" : eits_[hop_ - 1]->GetEdge().GetId())); \
  } while (0)
#else
#define VAR_LEN_EXP_DUMP_FOR_DEBUG()
#endif
...
class VarLenExpand : public OpBase {
  void _InitializeEdgeIter(RTContext *ctx, int64_t vid,
    ...
    auto types = std::unordered_set<std::string>{relp_->Types().begin(),
                                                 relp_->Types().end()};
    ...
    switch (expand_direction_) {
      ...
      case ExpandTowards::BIDIRECTIONAL:
        eit = std::make_unique<graphdb::ScanEdgeByVidDirectionTypes>(
            ctx->txn_, vid, graphdb::EdgeDirection::BOTH, std::move(type_set));
        break;
    }
  }

  std::optional<graphdb::Vertex> GetFirstFromKthHop(
      RTContext *ctx, std::optional<graphdb::Vertex> vertex, int k) {
    if (k == 1) {
      relp_->path_.Clear();
    ...
    while (eits_[k - 1]->Valid()) {
      if (!ctx->path_unique_ ||
          (ctx->path_unique_ && !pattern_graph_->VisitedEdges().count(
                                    eits_[k - 1]->GetEdge().GetId()))) {
        auto neighbor_vertex =
            eits_[k - 1]->GetEdge().GetOtherEnd(vertex->GetId());
        relp_->path_.Append(eits_[k - 1]->GetEdge(), neighbor_vertex);
        if (ctx->path_unique_) {
          pattern_graph_->VisitedEdges().insert(
              eits_[k - 1]->GetEdge().GetId());
        }
  ...
  std::optional<graphdb::Vertex> GetNextFromKthHop(RTContext *ctx, size_t k) {
    ...
    do {
      ...
      if (eits_[k - 1]->Valid() &&
        ...
        auto neighbor_vertex =
            eits_[k - 1]->GetEdge().GetOtherEnd(last.GetId());
        relp_->path_.Append(eits_[k - 1]->GetEdge(), neighbor_vertex);
        if (ctx->path_unique_) {
          pattern_graph_->VisitedEdges().insert(
              eits_[k - 1]->GetEdge().GetId());
  ...
  std::string ToString() const override {
    auto towards = expand_direction_ == FORWARD    ? "-->"
                   : expand_direction_ == REVERSED ? "<--"
                                                   : "--";
    return fmt::format("{}({}) [{} {}*{}..{} {}]", name, "All", start_->Alias(),
                       towards, std::to_string(min_hop_),
                       std::to_string(max_hop_), neighbor_->Alias());
  }
