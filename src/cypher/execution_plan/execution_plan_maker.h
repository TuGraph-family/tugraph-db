...
class ExecutionPlanMaker : public geax::frontend::AstNodeVisitor {
 public:
  ExecutionPlanMaker(std::vector<PatternGraph>& pattern_graphs,
      ...
      : pattern_graphs_(pattern_graphs), objAlloc_(alloc) {}
  ~ExecutionPlanMaker() override = default;
 ...
 private:
  ...
  // last_op_ is used to store the root node of the operators created during
  // pre-order traversal
 ...
 private:
  ...
  void _AddScanOp(const SymbolTable* sym_tab, Node* node,
