...
geax::frontend::GEAXErrorCode PatternGraphMaker::Build(
    geax::frontend::AstNode* astNode, RTContext* ctx) {
  ctx_ = ctx;
...
std::any PatternGraphMaker::visit(geax::frontend::PathPattern* node) {
  ...
  if (node->alias().has_value()) {
    ...
    auto& path_elements =
        pattern_graph.symbol_table.anot_collection.path_elements;
    ...
    for (idx = 0; idx < relationships.size(); ++idx) {
      std::shared_ptr<geax::frontend::Ref> node_ref =
          std::make_shared<geax::frontend::Ref>();
      alias = nodes[idx].Alias();
      node_ref->setName(std::move(alias));
      paths.push_back(node_ref);
      std::shared_ptr<geax::frontend::Ref> relp_ref =
          std::make_shared<geax::frontend::Ref>();
  ...
  if (ClauseGuard::InClause(geax::frontend::AstNodeType::kNode, cur_types_)) {
    ...
    if (it == symbols.end()) {
      if (pattern_graph.GetNode(variable.value()).Empty()) {
        ...
        } else if (ClauseGuard::InClause(
                       geax::frontend::AstNodeType::kInsertStatement,
                       cur_types_)) {
          node_t_->derivation_ = Node::Derivation::CREATED;
        } else if (ClauseGuard::InClause(
                       geax::frontend::AstNodeType::kMergeStatement,
                       cur_types_)) {
      ...
      if (ClauseGuard::InClause(geax::frontend::AstNodeType::kMergeStatement,
                                cur_types_)) {
...
std::any PatternGraphMaker::visit(geax::frontend::OrderByField*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::PathModePrefix*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::PathSearchPrefix*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::PropStruct* node) {
  if (ClauseGuard::InClause(geax::frontend::AstNodeType::kMatchStatement,
                            cur_types_) ||
      ClauseGuard::InClause(geax::frontend::AstNodeType::kMergeStatement,
                            cur_types_)) {
    if (ClauseGuard::InClause(geax::frontend::AstNodeType::kNode, cur_types_) ||
    ...
    NOT_SUPPORT();
                 ...
                 geax::frontend::AstNodeType::kInsertStatement, cur_types_)) {
    return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
  } else {
    NOT_SUPPORT();
  }
...
std::any PatternGraphMaker::visit(geax::frontend::YieldField* node) {
  ...
  for (auto& pair : node->items()) {
    ...
    if (!found) {
      THROW_CODE(CypherException, "No such yield field: {} in {}", name,
                 p->proc_name);
    }
    switch (type) {
      case ProcedureResultType::Value: {
        AddSymbol(name, cypher::SymbolNode::CONSTANT,
      ...
      case ProcedureResultType::Relationship: {
        CYPHER_TODO();
        AddSymbol(name, cypher::SymbolNode::RELATIONSHIP,
                  cypher::SymbolNode::LOCAL);
        break;
      }
      default:
        THROW_CODE(CypherException, "Unkown yield field type: {}", (int)type);
    }
  }
  if (node->predicate()) {
    ACCEPT_AND_CHECK_WITH_ERROR_MSG(node->predicate());
...
std::any PatternGraphMaker::visit(geax::frontend::TableFunctionClause*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::ReadConsistency*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::AllowAnonymousTable*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::OpConcurrent*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::EdgeOnJoin*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SetAllProperties*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::SetSingleProperty*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SetSchemaClause*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SetGraphClause*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SetTimeZoneClause*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SetParamClause*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::ResetAll*) { NOT_SUPPORT(); }

std::any PatternGraphMaker::visit(geax::frontend::ResetSchema*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::ResetTimeZone*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::ResetGraph*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::ResetParam*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::MultiCount*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::Param* node) {
  AddSymbol(node->name(), SymbolNode::Type::PARAMETER,
            cypher::SymbolNode::LOCAL);
  return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
}
...
std::any PatternGraphMaker::visit(geax::frontend::IsDirected*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::IsNormalized*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::IsSourceOf*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::IsDestinationOf*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::AllDifferent*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::Exists* node) {
  ...
  auto& symbols_prev =
      pattern_graphs_[cur_pattern_graph_ - 1].symbol_table.symbols;
  ...
  for (auto& [alias, symbol] : symbols_prev) {
    ...
    if (it != symbols_cur.end()) {
      argument_symbols.emplace(
          symbol.id, std::make_pair(alias, SymbolNode(symbol.id, symbol.type,
                                                      SymbolNode::ARGUMENT)));
    }
  }

  for (auto& [id, pair] : argument_symbols) {
    temp_symbols.emplace(
        pair.first,
  ...
  for (auto& [alias, symbol] : symbols_cur) {
    ...
    if (it == symbols_prev.end()) {
      temp_symbols.emplace(
          alias, SymbolNode(temp_symbols_idx++, symbol.type, symbol.scope));
    }
  }
...
std::any PatternGraphMaker::visit(geax::frontend::ExplainActivity*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SessionActivity*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::FullTransaction*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::StartTransaction*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::CommitTransaction*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::RollBackTransaction*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SessionSet*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::SessionReset*) {
  NOT_SUPPORT();
}
  ...
  for (auto& graph : pattern_graphs_) {
    ...
    for (auto& a : graph.symbol_table.symbols) {
      if (a.second.scope == SymbolNode::ARGUMENT) {
        if (a.second.type == SymbolNode::NODE &&
            graph.GetNode(a.first).Empty()) {
...
std::any PatternGraphMaker::visit(geax::frontend::SchemaFromPath*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::BindingValue*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::BindingGraph*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::BindingTable*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::BindingTableInnerQuery*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::BindingTableInnerExpr*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::JoinRightPart*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(
...
std::any PatternGraphMaker::visit(
  ...
  if (match_count > 1) {
    THROW_CODE(CypherException,
               "Not support more than one (optional) match clause.");
  }
...
std::any PatternGraphMaker::visit(geax::frontend::SelectStatement*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::FocusedQueryStatement*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::FocusedResultStatement*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::InlineProcedureCall*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::ForStatement*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(
  ...
  for (auto item : items) {
    ...
    if (cur_pattern_graph_ < pattern_graphs_.size() - 1 &&
      ...
      pattern_graph.symbol_table.symbols.emplace(
          alias, SymbolNode(symbols_idx_[cur_pattern_graph_]++, symbol_type,
                            SymbolNode::LOCAL));
    }
    if (!pattern_graph_in_union_[cur_pattern_graph_]) {
      if (cur_pattern_graph_ < pattern_graphs_.size() - 1 &&
          pattern_graphs_[cur_pattern_graph_ + 1].symbol_table.symbols.find(
              alias) == pattern_graphs_[cur_pattern_graph_ + 1]
                            .symbol_table.symbols.end()) {
        pattern_graphs_[cur_pattern_graph_ + 1].symbol_table.symbols.emplace(
            alias, SymbolNode(symbols_idx_[cur_pattern_graph_ + 1]++,
                              symbol_type, SymbolNode::ARGUMENT));
      }
    }
  }
  return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
...
std::any PatternGraphMaker::visit(geax::frontend::CatalogModifyStatement*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::ReplaceStatement*) {
  NOT_SUPPORT();
}
...
std::any PatternGraphMaker::visit(geax::frontend::ShowProcessListStatement*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::KillStatement*) {
  NOT_SUPPORT();
}

std::any PatternGraphMaker::visit(geax::frontend::ManagerStatement*) {
  NOT_SUPPORT();
}
...
void PatternGraphMaker::AddSymbol(const std::string& symbol_alias,
                                  cypher::SymbolNode::Type type,
  ...
  table.emplace(symbol_alias,
                SymbolNode(symbols_idx_[cur_pattern_graph_]++, type, scope));
}
...
std::any PatternGraphMaker::visit(geax::frontend::RemoveSingleProperty*) {
  NOT_SUPPORT();
}
std::any PatternGraphMaker::visit(geax::frontend::ListComprehension* node) {
  ...
  AddSymbol(ref->name(), cypher::SymbolNode::CONSTANT,
            cypher::SymbolNode::LOCAL);
