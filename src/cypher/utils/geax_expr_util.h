...
class AstExprToString : public geax::frontend::AstExprNodeVisitorImpl {
 ...
 private:
  ...
  std::any visit(geax::frontend::TupleGet*) override {
    NOT_SUPPORT_AND_THROW();
  }
  std::any visit(geax::frontend::Not* node) override {
    EXP_UNARY_EXPR_TOSTRING("!");
  }
  ...
  std::any visit(geax::frontend::BSafeEqual*) override {
    NOT_SUPPORT_AND_THROW();
  }
  ...
  std::any visit(geax::frontend::BBitLeftShift*) override {
    NOT_SUPPORT_AND_THROW();
  }
  std::any visit(geax::frontend::BBitRightShift*) override {
    NOT_SUPPORT_AND_THROW();
  }
  ...
  std::any visit(geax::frontend::MatchCase*) override {
    NOT_SUPPORT_AND_THROW();
  }
  std::any visit(geax::frontend::AggFunc*) override { NOT_SUPPORT_AND_THROW(); }
  std::any visit(geax::frontend::BAggFunc*) override {
    NOT_SUPPORT_AND_THROW();
  }
  std::any visit(geax::frontend::MultiCount*) override {
    NOT_SUPPORT_AND_THROW();
  }
  std::any visit(geax::frontend::Windowing*) override {
    NOT_SUPPORT_AND_THROW();
  }
  ...
  std::any visit(geax::frontend::ListComprehension* node) override {
    std::string res("{");
    const auto& exprs =
        std::vector{node->getVariable(), node->getInExpression(),
                    node->getWhereExpression(), node->getOpExpression()};
    for (size_t idx = 0; idx < exprs.size(); ++idx) {
      if (exprs[idx] != nullptr) {
        std::string temp;
        temp = std::any_cast<std::string>(exprs[idx]->accept(*this));
        res += temp;
        if (idx != exprs.size() - 1) {
  ...
  std::any visit(geax::frontend::MkMap*) override { NOT_SUPPORT_AND_THROW(); }
  std::any visit(geax::frontend::MkRecord*) override {
    NOT_SUPPORT_AND_THROW();
  }
...
//
// gcc would flag an unused static function, but not an inline static one. For
// clang though, the outcome is different.
//
[[maybe_unused]] inline static std::string ToString(
    geax::frontend::Expr* node) {
