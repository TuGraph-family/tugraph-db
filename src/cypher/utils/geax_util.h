...
#define VISIT_AND_CHECK_WITH_ERROR_MSG(ast)                                 \
  do {                                                                      \
    ...
    if (res != geax::frontend::GEAXErrorCode::GEAX_SUCCEED) {               \
      error_msg_ =                                                          \
          error_msg_.empty()                                                \
              ? fmt::format("visit({}) failed at {}:{}", std::string(#ast), \
...
#define ACCEPT_AND_CHECK_WITH_ERROR_MSG(ast)                                \
  do {                                                                      \
    if (!ast) NOT_SUPPORT();                                                \
    auto res =                                                              \
        std::any_cast<geax::frontend::GEAXErrorCode>(ast->accept(*this));   \
    if (res != geax::frontend::GEAXErrorCode::GEAX_SUCCEED) {               \
      error_msg_ =                                                          \
          error_msg_.empty()                                                \
              ? fmt::format("visit({}) failed at {}:{}", std::string(#ast), \
...
#define NOT_SUPPORT()                                                         \
  do {                                                                        \
    error_msg_ =                                                              \
        error_msg_.empty()                                                    \
            ? fmt::format("failed at {}:{}", std::string(__FILE__), __LINE__) \
            : error_msg_;                                                     \
...
#define NOT_SUPPORT_AND_THROW()                                    \
  do {                                                             \
    auto error_msg = fmt::format("visit(...) failed at {}:{}",     \
                                 std::string(__FILE__), __LINE__); \
    THROW_CODE(CypherException, error_msg);                        \
  } while (0)
#endif
