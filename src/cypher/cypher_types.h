...
struct FieldDatav1 {
  typedef std::unordered_map<std::string, cypher::FieldDatav1>
      CYPHER_FIELD_DATA_MAP;
  ...
  explicit FieldDatav1(const char* rhs, size_t s)
      : type(SCALAR), scalar(std::string(rhs, s)) {}
  ...
  bool operator==(const FieldDatav1& rhs) const {
    if (type != rhs.type)
      throw std::runtime_error(
          "Unable to compare between different field data types");
    switch (type) {
      case ARRAY:
        CYPHER_TODO();
      case MAP:
  ...
  bool IsInteger() const { return type == SCALAR && scalar.IsInteger(); }

  bool IsReal() const { return type == SCALAR && scalar.IsDouble(); }

  bool IsString() const { return type == SCALAR && scalar.IsString(); }

  bool IsArray() const { return type == ARRAY; }

  static FieldDatav1 Array(size_t n) {
    return FieldDatav1(std::vector<cypher::FieldDatav1>(n));
  }
  ...
  std::string ToString() const {
    ...
    throw std::runtime_error("internal error: unhandled type: " +
                             std::to_string((int)type));
  }
  inline bool AsBool() const { return scalar.AsBool(); }

  inline int64_t AsInteger() const { return scalar.AsInteger(); }

  inline double AsDouble() const { return scalar.AsDouble(); }

  inline std::string AsString() const { return scalar.AsString(); }
...
enum PredicateType { None = 1, Single, Any, All };
