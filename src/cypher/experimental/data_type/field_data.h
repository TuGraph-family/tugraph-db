...
struct CScalarData {
    ...
    std::variant<std::monostate,  // Represent the null state
                 dyn_var<int32_t>, dyn_var<int>, dyn_var<int64_t>, dyn_var<float>, dyn_var<double>>
        constant_;

    lgraph::FieldType type_;

    CScalarData() { type_ = lgraph_api::FieldType::NUL; }

    CScalarData(CScalarData&& data) : constant_(std::move(data.constant_)), type_(data.type_) {}

    CScalarData(const CScalarData& other) : constant_(other.constant_), type_(other.type_) {}
    ...
    explicit CScalarData(const static_var<int64_t>& integer) : type_(FieldType::INT64) {
    ...
    dyn_var<int64_t> Int64() const { return std::get<dyn_var<int64_t>>(constant_); }

    inline bool is_integer() const { return type_ >= FieldType::INT8 && type_ <= FieldType::INT64; }

    inline bool is_real() const { return type_ == FieldType::DOUBLE || type_ == FieldType::FLOAT; }
...
struct CFieldData {
    ...
    explicit CFieldData(CScalarData&& scalar) : type(SCALAR), scalar(std::move(scalar)) {}
