...
static bool Entry2Double(const Entry &e, double &d) {
  ...
  if (e.IsInteger()) {
    d = static_cast<double>(e.constant.AsInteger());
  } else if (e.IsReal()) {
    d = e.constant.AsDouble();
  } else {
    return false;
  }
  return true;
...
class AvgAggCtx : public AggCtx {
 ...
 public:
  ...
  int ReduceNext() override {
    if (count > 0) {
      result = Entry(Value(total / count));
    } else {
      result = Entry(Value());
    }
    return 0;
...
class CountAggCtx : public AggCtx {
  ...
  std::unordered_set<Value, ValueHash> uset;
 ...
 public:
  int Step(const std::vector<Entry> &args) override {
    ...
    if (distinct < 0) distinct = args[0].constant.AsBool() ? 1 : 0;
    if (!args[1].EqualNull()) {
      if (distinct > 0) {
        switch (args[1].type) {
          case Entry::CONSTANT:
            if (args[1].IsScalar()) {
              uset.emplace(args[1].constant);
            } else {
              CYPHER_TODO();
              // uset.emplace(PropertyValue(args[1].ToString()));
            }
            break;
          case Entry::NODE:
            // CYPHER_TODO();
            uset.emplace(args[1].node->vertex_->GetId());
            break;
          case Entry::RELATIONSHIP:
            // CYPHER_TODO();
            uset.emplace(args[1].relationship->edge_->GetId());
            break;
          default:
            CYPHER_TODO();
            // uset.emplace(PropertyValue(args[1].ToString()));
  ...
  int ReduceNext() override {
    ...
    result.type = Entry::CONSTANT;
    result.constant = Value::Integer(count);
    return 0;
  }
};
...
class CollectAggCtx : public AggCtx {
 ...
 public:
  CollectAggCtx() { result = Entry(Value::Array({})); }

  int Step(const std::vector<Entry> &args) override {
    CYPHER_THROW_ASSERT(args.size() == 2 && args[0].IsBool() &&
                        args[1].type == Entry::CONSTANT);
    distinct = args[0].constant.AsBool();
    for (size_t i = 1; i < args.size(); i++) {
      if (!args[i].constant.IsNull()) {
        if (distinct) {
          auto ret = uset.emplace(args[i].constant.ToString());
          if (!ret.second) continue;
        }
        result.constant.AsArray().emplace_back(args[i].constant);
        /*if (args[i].IsArray()) {
...
class PercentileDiscAggCtx : public AggCtx {
 ...
 public:
  PercentileDiscAggCtx() : values_allocated(1024) {
  ...
  int Step(const std::vector<Entry> &args) override {
    ...
    if (percentile < 0) {
      ...
      if (percentile < 0 || percentile > 1) {
        err =
            "PERCENTILE_DISC Invalid input for percentile is not a valid "
            "argument,"
  ...
  int ReduceNext() override {
    ...
    result.type = Entry::CONSTANT;
    result.constant = Value(values[idx]);
    return 0;
  }
};
...
class PercentileContAggCtx : public AggCtx {
 ...
 public:
  PercentileContAggCtx() : values_allocated(1024) {
    values.resize(values_allocated);
  }
  ...
  int Step(const std::vector<Entry> &args) override {
    ...
    if (percentile < 0) {
      ...
      if (percentile < 0 || percentile > 1) {
        err =
            "PERCENTILE CONT Invalid input for percentile is not a valid "
            "argument,"
            " must be a number in the range 0.0 to 1.0";
        return 1;
      }
  ...
  int ReduceNext() override {
    ...
    if (percentile == 1.0 || count == 1) {
      result.constant = Value(values[count - 1]);
      return 0;
    }
    ...
    if (!fraction_val) {
      ...
      result.constant = Value(values[index]);
      return 0;
    }
...
class VarianceAggCtx : public AggCtx {
 ...
 public:
  ...
  int ReduceNext() override {
    ...
    // is_sampled will be equal to 1 in the Varaince case and 0 in the VarianceP
    // case
    double variance = sum / (count - is_sampled);
    result.constant = Value(variance);
    return 0;
  }
};
...
class VariancePAggCtx : public AggCtx {
 ...
 public:
  VariancePAggCtx() : values_allocated(1024) {
    values.resize(values_allocated);
  }
  ...
  int ReduceNext() override {
    ...
    // is_sampled will be equal to 1 in the Variance case and 0 in the VarianceP
    // case
    double variance = sum / (count - is_sampled);
    result.constant = Value(variance);
    return 0;
