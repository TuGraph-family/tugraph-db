...
void PatternGraph::_CollectExpandStepsByDFS(NodeID start, bool ignore_created,
  ...
  while (!sn.empty()) {
    ...
    for (auto rr : curr.RhsRelps()) {
      ...
      if (!nbr.Visited() &&
          (!ignore_created || (nbr.derivation_ != Node::CREATED &&
...
Relationship &PatternGraph::GetRelationship(const std::string &alias) {
  ...
  return it == _relp_map.end() ? EmptyRelationship()
                               : _relationships[it->second];
}

const Relationship &PatternGraph::GetRelationship(
...
RelpID PatternGraph::AddRelationship(const std::set<std::string> &types,
...
RelpID PatternGraph::AddRelationship(const std::set<std::string> &types,
                                     geax::frontend::Edge *ast_node_,
                                     NodeID lhs, NodeID rhs,
                                     parser::LinkDirection direction,
                                     const std::string &alias, int min_hop,
                                     int max_hop,
                                     Relationship::Derivation derivation) {
  RelpID rid = _next_rid;
  _relationships.emplace_back(rid, types, ast_node_, lhs, rhs, direction, alias,
                              min_hop, max_hop, derivation);
  ...
  for (auto &r : _relationships) {
    ...
    auto derivation = r.derivation_ == Relationship::CREATED    ? "(CREATED)"
                      ...
                      : r.derivation_ == Relationship::ARGUMENT ? "(ARGUMENT)"
                                                                : "(UNKNOWN)";
    line.append(fmt::format("R[{} {} {}] {}:{} {}\n", std::to_string(r.Lhs()),
                            direction, std::to_string(r.Rhs()), r.Alias(),
                            types, derivation));
  }
