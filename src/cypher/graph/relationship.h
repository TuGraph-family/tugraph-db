...
class Relationship {
 ...
 public:
  ...
  std::optional<graphdb::Edge> edge_;
  int min_hop_ = -1, max_hop_ = -1;  // for variable length relationship
  Path path_;                        // for variable length relationship

  Relationship();

  Relationship(RelpID id, const std::set<std::string> &types, NodeID lhs,
               NodeID rhs, parser::LinkDirection direction,
               const std::string &alias, Derivation derivation);

  Relationship(RelpID id, const std::set<std::string> &types,
               geax::frontend::Edge *ast_node_, NodeID src, NodeID dst,
               parser::LinkDirection direction, const std::string &alias,
               int min_hop, int max_hop, Derivation derivation);
