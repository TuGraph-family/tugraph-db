...
class PatternGraph {
  ...
  std::unordered_set<int64_t> _visited_edges;
  ...
  void _CollectExpandStepsByDFS(NodeID start, bool ignore_created,
 ...
 public:
  ...
  NodeID AddNode(const std::string &label, const std::string &alias,
                 Node::Derivation derivation);

  NodeID AddNode(Node *node) {
    NodeID nid = _next_nid;
    _nodes.emplace_back(nid, node->Label(), node->Alias(), node->derivation_);
    _nodes.back().ast_node_ = node->ast_node_;
    _node_map.emplace(node->Alias(), nid);
    _next_nid++;
    return nid;
  }

  RelpID AddRelationship(const std::set<std::string> &types, NodeID lhs,
                         NodeID rhs, parser::LinkDirection direction,
                         const std::string &alias,
                         Relationship::Derivation derivation);

  RelpID AddRelationship(const std::set<std::string> &types,
                         geax::frontend::Edge *ast_node_, NodeID lhs,
                         NodeID rhs, parser::LinkDirection direction,
                         const std::string &alias, int min_hop, int max_hop,
                         Relationship::Derivation derivation);

  RelpID AddRelationship(Relationship *relp) {
    return AddRelationship(relp->Types(), relp->ast_node_, relp->Lhs(),
                           relp->Rhs(), relp->direction_, relp->Alias(),
                           relp->MinHop(), relp->MaxHop(), relp->derivation_);
  }

  std::vector<EXPAND_STEPS> CollectExpandStreams(
      const std::vector<NodeID> &start_nodes, bool ignore_created) {
  ...
  std::unordered_set<int64_t> &VisitedEdges() { return _visited_edges; }
};
}  // namespace cypher
