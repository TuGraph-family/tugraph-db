...
class NegativeSample:
    ...
    def Compute(self) -> cython.void:
        ...
        with cython.nogil, parallel():
            thread_id = omp_get_thread_num()
            begin = cython.cast(int, self.src_list.shape[0] / self.num_threads) * thread_id
            end = cython.cast(int, self.src_list.shape[0] / self.num_threads) * (thread_id + 1)
            if thread_id == self.num_threads - 1:
                end = self.src_list.shape[0]
            local_txn = self.db.ForkTxn(self.txn)
            vit = local_txn.GetVertexIterator()
            for i in range(begin, end):
                degree = cython.declare(size_t,  self.g.OutDegree(self.src_list[i]))
                if self.src_list[i] == self.dst_list[i]:continue
                if degree == 0:
                    if self.flag[self.src_list[i]] == 0:
                        self.g.AcquireVertexLock(self.src_list[i])
                        if self.flag[self.src_list[i]] == 0:
                            vit.Goto(self.src_list[i])
                            ...
                            self.local_node[thread_id, self.local_node_num[thread_id]] = self.src_list[i]
                            self.local_node_num[thread_id] += 1
                            self.flag[self.src_list[i]] = -1
                        self.g.ReleaseVertexLock(self.src_list[i])
                    if self.flag[self.dst_list[i]] == 0:
                        self.g.AcquireVertexLock(self.dst_list[i])
                        if self.flag[self.dst_list[i]] == 0:
                            vit.Goto(self.dst_list[i])
                            feat_string = vit.GetField(self.feature_key).ToString()
                            feature_list = cython.cast(cython.p_float, feat_string.c_str())
                            label_string = vit.GetField(self.label_key).AsInt32()
                            memcpy(cython.address(self.local_feature[thread_id, self.local_node_num[thread_id], 0]), feature_list, self.feature_num * cython.sizeof(cython.float))
                            self.local_label[thread_id, self.local_node_num[thread_id]] = label_string
                            self.local_node[thread_id, self.local_node_num[thread_id]] = self.dst_list[i]
                            self.local_node_num[thread_id] += 1
                            self.flag[self.dst_list[i]] = -1
                        self.g.ReleaseVertexLock(self.dst_list[i])
                else:
                    out_edges = cython.declare(AdjList[Empty], self.g.OutEdges(self.src_list[i]))
                    for j in range(degree):
                        dst = out_edges[i].neighbour
                        if dst == self.dst_list[i]:
                            break
                        if self.flag[self.src_list[i]] == 0:
                            self.g.AcquireVertexLock(self.src_list[i])
                            if self.flag[self.src_list[i]] == 0:
    ...
    @cython.cfunc
    @cython.exceptval(check=False)
    def run(self, db: cython.pointer(GraphDB), olapondb:cython.pointer(OlapOnDB[Empty]), feature_num: size_t, num_samples: int, NodeInfo: list):
        start = time.time()
        ...
        self.src_list = edges[0]
        self.dst_list = edges[1]
        cost = time.time()
...
@cython.ccall
def Process(db_: lgraph_db_python.PyGraphDB, olapondb:lgraph_db_python.PyOlapOnDB, feature_num: size_t, num_samples: size_t, NodeInfo: list):
    addr = cython.declare(cython.Py_ssize_t, db_.get_pointer())
    olapondb_addr = cython.declare(cython.Py_ssize_t, olapondb.get_pointer())
    a = NegativeSample()
    a.run(cython.cast(cython.pointer(GraphDB), addr), cython.cast(cython.pointer(OlapOnDB[Empty]), olapondb_addr), feature_num, num_samples, NodeInfo)
