...
class EdgeSample:
            ...
            if (dis(rng) < self.sample_rate):
                ...
                if self.flag[vi] == 0:
                    ...
                    if self.flag[vi] == 0:
                        ...
                        self.flag[vi] = cython.cast(size_t, 1)
                    self.g.ReleaseVertexLock(vi)
    ...
    def run(self, db: cython.pointer(GraphDB), olapondb:cython.pointer(OlapOnDB[Empty]), feature_num: size_t, sample_rate: cython.double, NodeInfo: list, EdgeInfo: list):
        ...
        self.active.Fill()
        cost = time.time()
        self.g.ProcessVertexActive[size_t, EdgeSample](self.Work, self.active, self)
