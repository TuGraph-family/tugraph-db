...
#define GEAX_ANY_OK(ret) \
  (GEAXErrorCode::GEAX_SUCCEED == (std::any_cast<GEAXErrorCode>(ret)))
#define GEAX_ANY_FAIL(ret) \
  (GEAXErrorCode::GEAX_SUCCEED != (std::any_cast<GEAXErrorCode>(ret)))

#define GEAX_RET_OK(statement) \
  (GEAXErrorCode::GEAX_SUCCEED == (ret = (statement)))
#define GEAX_RET_FAIL(statement) \
  (GEAXErrorCode::GEAX_SUCCEED != (ret = (statement)))

#define GEAX_ANY_RET_OK(statement) \
  (GEAXErrorCode::GEAX_SUCCEED ==  \
   (ret = (std::any_cast<GEAXErrorCode>(statement))))
