 ...
 *
 * We can transform exsting graph query languages of any kind into this
 * structured GPML, including ISO/GQL, SQL/PGQ, ORACLE/PGQL, cypher and gremlin,
...
class GraphPattern : public AstNode {
 public:
  ...
  void appendPathPattern(PathPattern* pathPattern) {
    pathPatterns_.emplace_back(pathPattern);
  }
  void setPathPatterns(std::vector<PathPattern*>&& pathPatterns) {
    pathPatterns_ = std::move(pathPatterns);
  }
  const std::vector<PathPattern*>& pathPatterns() const {
