...
class MatchCase : public Expr {
 public:
  ...
  void appendCase(Expr* cond, Expr* result) {
    cases_.emplace_back(cond, result);
  }
  void setCases(std::vector<std::tuple<Expr*, Expr*>>&& cases) {
...
inline bool MatchCase::equals(const Expr& other) const {
  ...
  bool ret = (nullptr != input_) && (nullptr != expr.input_) &&
             *input_ == *expr.input_;
  ret = ret && cases_.size() == expr.cases_.size();
  for (auto i = 0u; i < cases_.size() && ret; ++i) {
    ret = (nullptr != std::get<0>(cases_[i])) &&
          (nullptr != std::get<0>(expr.cases_[i])) &&
          (nullptr != std::get<1>(cases_[i])) &&
          (nullptr != std::get<1>(expr.cases_[i]));
