...
class Case : public Expr {
 public:
  ...
  void appendCaseBody(Expr* cond, Expr* result) {
    caseBodies_.emplace_back(cond, result);
  }
  void setCaseBodies(std::vector<std::tuple<Expr*, Expr*>>&& caseBodies) {
    caseBodies_ = std::move(caseBodies);
  }
  const std::vector<std::tuple<Expr*, Expr*>>& caseBodies() const {
    return caseBodies_;
  }
...
inline bool Case::equals(const Expr& other) const {
  ...
  for (auto i = 0u; i < caseBodies_.size() && ret; ++i) {
    ...
    ret = ret &&
          *std::get<0>(caseBodies_[i]) == *std::get<0>(expr.caseBodies_[i]);
    ret = ret &&
          (*std::get<1>(caseBodies_[i]) == *std::get<1>(expr.caseBodies_[i]));
  }
  return ret;
}
