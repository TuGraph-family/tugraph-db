...
class MkRecord : public Expr {
 public:
  ...
  void appendElem(std::string&& k, Expr* v) {
    elems_.emplace_back(std::move(k), v);
  }
  void setElems(std::vector<std::tuple<std::string, Expr*>>&& elems) {
    elems_ = std::move(elems);
  }
  const std::vector<std::tuple<std::string, Expr*>>& elems() const {
    return elems_;
  }
...
inline bool MkRecord::equals(const Expr& other) const {
  ...
  for (auto i = 0u; i < elems_.size() && ret; ++i) {
    ret = (nullptr != std::get<1>(elems_[i])) &&
          (nullptr != std::get<1>(expr.elems_[i]));
