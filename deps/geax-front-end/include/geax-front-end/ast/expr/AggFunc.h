...
inline const char* ToString(GeneralSetFunction type) {
  static const StrArray<enumNum(GeneralSetFunction::kLastButNotUse)> kDict = {
      ...
      "collect", "stdDevSamp", "stdDevPop", "groupConcat", "stDev",
      "stDevP",  "variance",   "varianceP"};
...
inline bool ToEnum(std::string_view sv, GeneralSetFunction& type) {
  static const std::unordered_map<std::string_view, GeneralSetFunction>
      kTypeMap = {{"avg", GeneralSetFunction::kAvg},
...
class AggFunc : public Expr {
 public:
  ...
  void setDistinctBy(std::vector<Expr*>&& distinctBy) {
    distinctBy_ = std::move(distinctBy);
  }
