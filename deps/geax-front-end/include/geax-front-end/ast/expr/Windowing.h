...
inline bool Windowing::equals(const Expr& other) const {
  ...
  for (auto i = 0u; i < orderByClause_.size() && ret; ++i) {
    ...
    ret = ret && *std::get<0>(orderByClause_[i]) ==
                     *std::get<0>(expr.orderByClause_[i]);
    ret = ret && (std::get<1>(orderByClause_[i]) ==
                  std::get<1>(expr.orderByClause_[i]));
  }
