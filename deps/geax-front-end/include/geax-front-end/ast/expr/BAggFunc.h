...
inline bool ToEnum(std::string_view sv, BinarySetFunction& type) {
  static const std::unordered_map<std::string_view, BinarySetFunction>
      kTypeMap = {
...
class BAggFunc : public Expr {
 public:
  ...
  void setLExpr(bool isDistinct, Expr* expr) {
    lExpr_ = std::make_tuple(isDistinct, expr);
  }
...
inline bool BAggFunc::equals(const Expr& other) const {
  ...
  bool ret = (nullptr != rExpr_) && (nullptr != expr.rExpr_) &&
             (nullptr != std::get<1>(lExpr_)) &&
             (nullptr != std::get<1>(expr.lExpr_));
  ret = ret && funcName_ == expr.funcName_ &&
        std::get<0>(lExpr_) == std::get<0>(expr.lExpr_) &&
        *std::get<1>(lExpr_) == *std::get<1>(expr.lExpr_) &&
        *rExpr_ == *expr.rExpr_;
  return ret;
}
