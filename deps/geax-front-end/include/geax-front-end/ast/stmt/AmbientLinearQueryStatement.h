...
/**
 * An AmbientLinearQueryStatement represents a stmt that have multiple prefixed
 * reading clauses and a trailing with/return clause.
 */
class AmbientLinearQueryStatement : public LinearQueryStatement {
 public:
  AmbientLinearQueryStatement()
      : LinearQueryStatement(AstNodeType::kAmbientStatement),
  ...
  void setQueryStatements(
      std::vector<SimpleQueryStatement*>&& queryStatements) {
    queryStatements_ = std::move(queryStatements);
  }
  const std::vector<SimpleQueryStatement*>& queryStatements() const {
    return queryStatements_;
  }
