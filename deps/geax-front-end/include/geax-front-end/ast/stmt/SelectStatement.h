...
class SelectStatement : public LinearQueryStatement {
 public:
  SelectStatement()
      : LinearQueryStatement(AstNodeType::kSelectStatement),
  ...
  void setFromClauses(
      std::vector<std::tuple<std::string, SimpleQueryStatement*>>&&
          fromClauses) {
    fromClauses_ = std::move(fromClauses);
  }
  const std::vector<std::tuple<std::string, SimpleQueryStatement*>>&
  fromClauses() const {
    return fromClauses_;
  }
