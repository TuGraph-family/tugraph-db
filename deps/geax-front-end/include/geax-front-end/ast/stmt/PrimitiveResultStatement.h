...
class PrimitiveResultStatement : public AstNode {
 public:
  ...
  void appendItem(std::string&& name, Expr* expr) {
    items_.emplace_back(std::move(name), expr, false);
  }
  void setItems(std::vector<std::tuple<std::string, Expr*, bool>>&& items) {
    items_ = std::move(items);
  }
  const std::vector<std::tuple<std::string, Expr*, bool>>& items() const {
    return items_;
  }
  void appendGroupKey(Expr* groupKey) { groupKeys_.emplace_back(groupKey); }
  void setGroupKeys(std::vector<Expr*>&& groupKeys) {
  ...
  void setOrderBys(std::vector<OrderByField*>&& orderBys) {
    orderBys_ = std::move(orderBys);
  }
