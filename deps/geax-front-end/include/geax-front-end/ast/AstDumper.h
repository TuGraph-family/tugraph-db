...
class VariableGuard {
 public:
  explicit VariableGuard(std::string& str, int64_t* indent,
...
class AstDumper : public AstNodeVisitor {
 public:
  ...
  std::any reportError(T) {
    error_msg_ = error_msg_.empty()
                     ? "visit(" + std::string(typeid(T).name()) +
                           ") failed at" + std::string(__FILE__) + ":" +
                           std::to_string(__LINE__)
                     : error_msg_;
    return reportError();
  }
  ...
  std::any visit(Edge* node) override {
    INDET_GUARD();
    VARIABLE_GUARD_WITH_TYPE_NAME(Edge);
  ...
  std::any visit(WhereClause* node) override {
    INDET_GUARD();
    VARIABLE_GUARD_WITH_TYPE_NAME(WhereClause);
  ...
  std::any visit(OrderByField* node) override {
    INDET_GUARD();
    VARIABLE_GUARD_WITH_TYPE_NAME(OrderByField);
  ...
  std::any visit(PropStruct* node) override {
    if (node) {
      INDET_GUARD();
      VARIABLE_GUARD_WITH_TYPE_NAME(PropStruct);
  ...
  std::any visit(YieldField* node) override {
    INDET_GUARD();
    VARIABLE_GUARD_WITH_TYPE_NAME(YieldField);
  ...
  std::any visit(TableFunctionClause* node) override {
    INDET_GUARD();
    VARIABLE_GUARD_WITH_TYPE_NAME(TableFunctionClause);
  ...
  std::any visit(Windowing* node) override {
    INDET_GUARD();
    VARIABLE_GUARD_WITH_TYPE_NAME(Windowing);
  ...
  std::any visit(InlineProcedureCall* node) override {
