...
GEAXErrorCode GQLParserTest::test(const std::string& testdir) {
  ...
  if (GEAX_RET_FAIL(
          loadTestFile(testdir.c_str(), testFilePattern, testFileMap))) {
...
std::vector<std::string> GQLParserTest::listAllFilesInDir(const char* dirname,
  ...
  if (dir == nullptr) {
    LOG(ERROR) << "Failed to read the directory \"" << dirname
               << "\": " << strerror(errno);
    return files;
  }
...
void GQLParserTest::dividePath(const std::string& path, std::string& parent,
                               std::string& child) {
  if (path.empty() || path == "/") {
...
GEAXErrorCode GQLParserTest::loadTestFile(
    const char* rootPath, const char* pattern,
    std::unordered_map<int32_t, TestFile>& testFileMap) {
  GEAXErrorCode ret = GEAXErrorCode::GEAX_SUCCEED;
