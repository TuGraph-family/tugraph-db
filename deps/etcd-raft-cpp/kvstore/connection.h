...
inline void ProtobufConnection::read_magic_done(const boost::system::error_code& ec) {
    if (ec) {
        LOG_WARN("read_magic_done error {}", ec.message());
        Close();
        return;
    }
...
inline void ProtobufConnection::read_msg_size_done(const boost::system::error_code &ec) {
    if (ec) {
        LOG_WARN("read_msg_size_done error {}", ec.message());
        Close();
        return;
    }
    ...
    if (msg_size_ > 1024 * 1024 * 1024) {
        LOG_WARN("receive message which is too big, size : {}", msg_size_);
        Close();
        return;
    }
    if (msg_size_ == 0) {
        LOG_WARN("receive message with error size, size : {}", msg_size_);
        Close();
        return;
    }
...
inline void ProtobufConnection::read_msg_body_done(const boost::system::error_code &ec) {
    if (ec) {
        LOG_WARN("read_msg_body_done error {}", ec.message().c_str());
        Close();
        return;
    }
    raftpb::Message msg;
    ...
    if (!ret) {
        LOG_WARN("raft msg ParseFromArray failed, close connection");
        Close();
        return;
    }
