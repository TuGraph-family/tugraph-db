...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
  g++ -fno-gnu-unique -fPIC -g --std=c++17 $LG_INCLUDE_CMD -rdynamic -O3 -fopenmp -DNDEBUG -o cpp/${PLUGIN_NAME}_standalone cpp/${PLUGIN_NAME}_core.cpp cpp/${PLUGIN_NAME}_standalone.cpp $SRC -lrt -lstdc++fs
elif [[ "$OSTYPE" == "darwin"* ]]; then
  clang++ -stdlib=libc++ -fPIC -g --std=c++17 $LG_INCLUDE_CMD -rdynamic -O3 -Xpreprocessor -fopenmp -lomp -DNDEBUG -o cpp/${PLUGIN_NAME}_standalone cpp/${PLUGIN_NAME}_core.cpp cpp/${PLUGIN_NAME}_standalone.cpp $SRC -lstdc++fs
fi
