...
`log_dir`配置项默认为空。若`log_dir`为空，则所有日志会输出到控制台(daemon模式下不指定log_dir为空不会向console输出任何日志)；若手动指定`log_dir`，则日志文件会生成在对应的路径下面，单个日志文件最大大小为256MB。
...
#### 2.3.1.python存储过程
请使用python自带的print输出调试信息，调试信息会在存储过程运行结束后统一输出至日志文件中，形成一条级别为`WARN`的日志。
