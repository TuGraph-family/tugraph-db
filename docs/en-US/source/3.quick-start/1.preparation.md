...
To switch between the old and new frontend versions, enter the container and modify the /usr/local/etc/lgraph.json configuration file. Use "web" : "/usr/local/share/lgraph/resource" for the old version or "web" : "/usr/local/share/lgraph/browser-resource" for the new version. After updating the configuration file, restart the container with docker restart tugraph_demo for the changes to take effect.The new version is the default option.
