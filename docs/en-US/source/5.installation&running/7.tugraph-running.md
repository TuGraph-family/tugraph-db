...
The new or old version can be selected through the configuration file at /usr/local/etc/lgraph.json, by setting "web" : "/usr/local/share/lgraph/resource" (for the old version) or "web" : "/usr/local/share/lgraph/browser-resource" (for the new version). Changes to the configuration file take effect after the service is restarted. The new version is the default option.
