...
class TestFaiss : public TuGraphTest {
 protected:
    int64_t dim = 4;
    ...
    std::vector<int> index_spec = {24};
    TestFaiss() : vectors(num_vectors, std::vector<float>(dim)), vids(num_vectors) {}
    void SetUp() override {
        std::mt19937 rng(47);
        ...
        vector_index =
            std::make_unique<lgraph::IVFFlat>
                            ("label", "name", "l2", "ivf_flat", dim, index_spec);
    }
...
TEST_F(TestFaiss, SaveAndLoadIndex) {
    ...
    lgraph::IVFFlat vector_index_loaded
                    ("label", "name", "l2", "ivf_flat", dim, index_spec);
...
TEST_F(TestFaiss, VectorProcedure) {
    using namespace lgraph;
    lgraph::GlobalConfig conf;
    conf.db_dir = "./testdb";
    conf.http_port = 7774;
    ...
    UT_EXPECT_TRUE(ret);
    UT_EXPECT_EQ(ElementCount_faiss_vector(json_val, "person", "label_name"), 1);
    ...
    ret = client.CallCypher(str,
    "CALL db.vertexVectorKnnSearch('person','vector',[1,2,3,4], "
                            "{top_k:2, ivf_flat_nprobe:10}) YIELD node RETURN node.id");
    UT_EXPECT_TRUE(ret);
    json_val = web::json::value::parse(str);
    UT_EXPECT_TRUE(ret);
