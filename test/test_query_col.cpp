...
class TestQueryCol : public TuGraphTest {
 private:
...
//                }
                ...
                // UT_LOG() << result;
                // UT_LOG() << "-----MY result-----";
                // ctx_->data_chunk_->Print();
                UT_LOG() << "-----MY result-----";
                result = ctx_->data_chunk_->Dump(false);
                // UT_LOG() << "-----ORIGINAL result-----";
                // result = ctx_->result_->Dump(false);
                // UT_LOG() << "-----result-----";
                // result = ctx_->result_->Dump(false);
                UT_LOG() << result;
    ...
    void load_procedure(const std::string& name, const std::string& procedure_source_path,
        ...
        // Public method to run the core test code
    void RunTestDemo() {
        set_graph_type(_ut_graph_dataset_type);
        set_query_type(_ut_query_type);
        std::string dir = lgraph::ut::TEST_RESOURCE_DIRECTORY + "/cases/demo";
        test_file(dir, false);
    }
};

TEST_F(TestQueryCol, TestDemo) {
    set_graph_type(_ut_graph_dataset_type);
    set_query_type(_ut_query_type);
