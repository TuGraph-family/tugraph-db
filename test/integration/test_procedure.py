...
class TestProcedure():
    ...
    def test_edge_label(self, server, client):
        ...
        assert ret[0]
        labels = json.loads(ret[1])
        assert labels.get("edgeLabels") == "married"
    ...
    def test_create_label(self, server, client):
        ...
        assert ret[0]
        labels = json.loads(ret[1])
        assert labels.get("label") == "animal"
    ...
    def test_rule(self, server, client):
        ...
        assert ret[0]
        info = json.loads(ret[1]).get("role_info")
        ...
        ret = client.callCypher("CALL dbms.security.getRoleInfo('test_role2')")
        assert ret[0]
        permissions = json.loads(ret[1]).get("role_info").get("permissions")
        ...
        assert ret[0]
        roles = json.loads(ret[1])
        assert roles.get("role_name") == "admin"
    ...
    def test_user(self, server, client):
        ...
        assert ret[0]
        info = json.loads(ret[1]).get("user_info")
        ...
        assert ret[0]
        roles = json.loads(ret[1]).get("user_info").get("roles")
        assert len(roles) == 3
