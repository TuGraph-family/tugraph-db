...
//         CheckParseDataType<int32_t>(FieldType::INT32, value_tmp, "6789", FieldData(-(1377 <<
//         20)),
...
//         CheckParseDataType<float>(FieldType::FLOAT, value_tmp, "12.125", FieldData(-13.5),
//         "float",
//                                   FieldData("a1234"));
//         CheckParseDataType<double>(FieldType::DOUBLE, value_tmp, "1332", FieldData(-13),
//         "double",
//                                    FieldData("d1234"));
...
//         CheckParseDataType<lgraph::DateTime>(FieldType::DATETIME, value_tmp, "2019-09-08
//         00:15:14",
//                                              FieldData::DateTime("2020-05-04 20:24:34"),
//                                              "not valid date", FieldData(123), true,
//                                              "10240-09-01");
...
void test_conversion(T& result, U& value, T expected) {
    _detail::FieldExtractor::ConvertData(&result, reinterpret_cast<char*>(&value), sizeof(value));
    UT_EXPECT_EQ(result, expected);
}
TEST_F(TestFieldExtractor, ConvertDataTest) {
    int8_t i8 = 100;
    int16_t i16p = std::numeric_limits<int8_t>::max() + 1;
    int16_t i16n = std::numeric_limits<int8_t>::min() - 1;
    int32_t i32p = std::numeric_limits<int16_t>::max() + 1;
    int32_t i32n = std::numeric_limits<int16_t>::min() - 1;
    int64_t i64p = static_cast<int64_t>(std::numeric_limits<int32_t>::max()) + 1;
    int64_t i64n = static_cast<int64_t>(std::numeric_limits<int32_t>::min()) - 1;
    int8_t resulti8;
    test_conversion(resulti8, i8, i8);
    test_conversion(resulti8, i16p, std::numeric_limits<int8_t>::max());
    test_conversion(resulti8, i16n, std::numeric_limits<int8_t>::min());
    int16_t resulti16;
    test_conversion(resulti16, i32p, std::numeric_limits<int16_t>::max());
    test_conversion(resulti16, i32n, std::numeric_limits<int16_t>::min());

    int32_t resulti32;
    test_conversion(resulti32, i64p, std::numeric_limits<int32_t>::max());
    test_conversion(resulti32, i64n, std::numeric_limits<int32_t>::min());

    double fdp = std::numeric_limits<float>::max();
    double fdn = std::numeric_limits<float>::min();
    float resultf;
