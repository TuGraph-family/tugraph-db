...
protected:
    ...
    void SetUp() override {
        ...
        vector_index = std::make_unique<lgraph::IVFFlat>("label", "name", "L2", "IVF_FLAT", dim, index_spec, nullptr);
        ASSERT_TRUE(vector_index->Build());
    }
    void TearDown() override {}
};

TEST_F(FaissIndexTest, BuildIndex) {
    ASSERT_TRUE(vector_index->Build());
}

TEST_F(FaissIndexTest, AddVectors) {
...
TEST_F(FaissIndexTest, SaveAndLoadIndex) {
    ...
    lgraph::IVFFlat vector_index_loaded("label", "name", "L2", "IVF_FLAT", dim, index_spec, nullptr);
