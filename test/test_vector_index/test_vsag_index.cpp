...
protected:
    ...
    VSAGTest() 
        : vectors(num_vectors, std::vector<float>(dim)), 
          vids(num_vectors) 
    {}
    void SetUp() override {
        std::mt19937 rng(47);
        ...
        vector_index = std::make_unique<lgraph::HNSW>("label", "name", "L2", "HNSW", dim, index_spec, nullptr);
    }
    void TearDown() override {}
};

TEST_F(VSAGTest, BuildIndex) {
...
TEST_F(VSAGTest, SaveAndLoadIndex) {
    ...
    lgraph::HNSW vector_index_loaded("label", "name", "L2", "HNSW", dim, index_spec, nullptr);
...
TEST_F(VSAGTest, DeleteVectors) {
    ...
    for (const auto& idx : indices) {
        ASSERT_TRUE(std::find(delete_vids.begin(), delete_vids.end(), idx) == delete_vids.end());
    }
}
