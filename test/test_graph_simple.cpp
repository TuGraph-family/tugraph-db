...
void DumpTable(KvTransaction& txn, KvTable& t, bool track_incoming) {
    t.Print(
        txn, [](const Value& k) -> std::string { return std::to_string(0); },
        [track_incoming](const Value& v) -> std::string { return ""; });
}
...
TEST_F(TestGraphSimple, GraphSimple) {
    ...
    txn = store.CreateReadTxn();
    {
        ...
        dsts = graph.GetUnmanagedVertexIterator(&txn, 0).ListDstVids();
        UT_EXPECT_EQ(dsts.size(), 0);
        dsts = graph.GetUnmanagedVertexIterator(&txn, 1).ListDstVids();
        UT_EXPECT_EQ(dsts.size(), 1);
        size_t ne = 0;
        dsts = graph.GetUnmanagedVertexIterator(&txn, 2).ListDstVids(
            nullptr, nullptr, std::numeric_limits<size_t>::max(), nullptr, &ne);
        UT_EXPECT_EQ(dsts.size(), 2);
        UT_EXPECT_EQ(ne, 6);

        std::vector<VertexId> srcs;
        srcs = graph.GetUnmanagedVertexIterator(&txn, 0).ListSrcVids();
        UT_EXPECT_EQ(srcs.size(), 0);
        srcs = graph.GetUnmanagedVertexIterator(&txn, 1).ListSrcVids();
        UT_EXPECT_EQ(srcs.size(), 0);
        UT_LOG() << "1 <- " << fma_common::ToString(srcs);
        srcs = graph.GetUnmanagedVertexIterator(&txn, 2).ListSrcVids();
        UT_EXPECT_EQ(srcs.size(), 2);
        UT_LOG() << "2 <- " << fma_common::ToString(srcs);
        srcs = graph.GetUnmanagedVertexIterator(&txn, 3).ListSrcVids();
        UT_EXPECT_EQ(srcs.size(), 1);
        UT_LOG() << "3 <- " << fma_common::ToString(srcs);
        auto vit = graph.GetUnmanagedVertexIterator(&txn);
        while (vit.IsValid()) {
            std::string line;
    ...
    auto vit = graph.GetUnmanagedVertexIterator(&txn, 3);
    auto eit = vit.GetInEdgeIterator();
    eit.Next();
    ...
    {
        auto it = graph.GetUnmanagedVertexIterator(&txn);
