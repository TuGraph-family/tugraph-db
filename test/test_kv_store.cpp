...
TEST_F(TestKvStore, KvStore) {
...
#if (!LGRAPH_USE_MOCK_KV)
    {
        ...
        AutoCleanDir dir1("./testkv");
        {
            ...
            txn = store.CreateWriteTxn();
            UT_EXPECT_EQ(txn.LastOpId(), -1);
            KvStore::SetLastOpIdOfAllStores(100);
...
#if (!LGRAPH_USE_MOCK_KV)
    try {
        KvStore store("./testkv", (size_t)1 << db_size, true);
        {
            CleanStore(store);
            UT_LOG() << "Testing iterator refresh";
            auto txn = store.CreateWriteTxn();
            KvTable tb1 = store.OpenTable(txn, "tb1", true, ComparatorDesc::DefaultComparator());
            txn.Commit();
            txn = store.CreateWriteTxn();
            {
            ...
            txn = store.CreateWriteTxn();
            {
                UT_EXPECT_TRUE(tb1.AddKV(txn, Value::ConstRef("key2"), Value::ConstRef("value2")));
                ...
                for (int i = 0; i < 1000; i++) {
                    ...
                    UT_EXPECT_TRUE(tb1.AddKV(txn, Value::ConstRef(key), Value::ConstRef("value")));
                }
                UT_EXPECT_TRUE(it2.RefreshAfterModify());
                UT_EXPECT_EQ(it2.GetValue().AsString(), "value2");
                ...
                // delete before it
                size_t i = 0;
                for (auto it = tb1.GetIterator(txn); it.IsValid();) {
                    i++;
        ...
        try {
            ...
            {
                KvTable tb1 =
                    store.OpenTable(txn, "tb1", true, ComparatorDesc::DefaultComparator());
                UT_EXPECT_TRUE(tb1.AddKV(txn, Value::ConstRef("key1"), Value::ConstRef("value1")));
    ...
    try {
        ...
        // test SetFixedSizeValue
        {
            txn = store.CreateWriteTxn();
            {
                ...
                UT_EXPECT_EQ(it.GetValue().AsString(), v2);
                UT_EXPECT_TRUE(it.AddKeyValue(Value::ConstRef<int>(2), Value::ConstRef(v2), false));
                UT_EXPECT_TRUE(it.AddKeyValue(Value::ConstRef<int>(3), Value::ConstRef(v2), false));
                UT_EXPECT_TRUE(it.AddKeyValue(Value::ConstRef<int>(4), Value::ConstRef(v2), false));
                UT_EXPECT_TRUE(it.IsValid());
                it.Prev();
                it.Prev();
                UT_EXPECT_TRUE(it.IsValid());
...

        {
            ...
            txn = store.CreateReadTxn();
            {
                ...
                for (int i = 0; i < nt; i++) {
                    threads.emplace_back([&txn, &store, &n_keys, &table, i]() {
                        auto ntxn = txn.Fork();
